      ...
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            specialitiesArray = [];
            _context.next = 3;
            return _fbConfig.default.firestore().collection("Specialities").get();
...
var AddCardFunction = function AddCardFunction(state, dispatch) {
  _fbConfig.default.firestore().collection("Specialities").add(_objectSpread({}, state)).then(function () {
    dispatch({
      type: "ADD_CARD",
